...
{
  wabi_map m = wabi_map_empty(&store);
  wabi_val k = (wabi_val) wabi_binary_leaf_new_from_cstring(&store, "key");
  wabi_val v = (wabi_val) wabi_binary_leaf_new_from_cstring(&store, "value");
  m = wabi_map_assoc(&store, m, k, v);
...
START_TEST(assoc_order)
{
  ...
  k = wabi_binary_leaf_new_from_cstring(&store, "key1");
  v = wabi_binary_leaf_new_from_cstring(&store, "val1");
  m = wabi_map_assoc(&store, m, (wabi_val) k, (wabi_val) v);
  k = wabi_binary_leaf_new_from_cstring(&store, "key2");
  v = wabi_binary_leaf_new_from_cstring(&store, "val2");
  m = wabi_map_assoc(&store, m, (wabi_val) k, (wabi_val) v);

  k = wabi_binary_leaf_new_from_cstring(&store, "key2");
  v = wabi_binary_leaf_new_from_cstring(&store, "val2");
  m0 = wabi_map_assoc(&store, m0, (wabi_val) k, (wabi_val) v);
  k = wabi_binary_leaf_new_from_cstring(&store, "key1");
  v = wabi_binary_leaf_new_from_cstring(&store, "val1");
  m0 = wabi_map_assoc(&store, m0, (wabi_val) k, (wabi_val) v);
...
START_TEST(iter)
{
  ...
  k = (wabi_val) wabi_binary_leaf_new_from_cstring(&store, "1");
  v = (wabi_val) wabi_binary_leaf_new_from_cstring(&store, "one");
  m = wabi_map_assoc(&store, m, k, v);
  wabi_map_iterator_init(&iter, m);
...
START_TEST(iter_hash)
{
  ...
  for(int j = 0; j < len; j++) {
    sprintf(str, "K%i", j);
    k = (wabi_val) wabi_binary_leaf_new_from_cstring(&store, str);
    sprintf(str, "V%i", j);
    v = (wabi_val) wabi_binary_leaf_new_from_cstring(&store, str);
    m = wabi_map_assoc(&store, m, k, v);
  }
  wabi_map_iterator_init(&iter, m);
  int cnt = 0;
