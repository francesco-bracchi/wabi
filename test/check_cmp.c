...
START_TEST(types)
{
  ...
  *nil = wabi_val_nil;
  ...
  wabi_val zero = (wabi_val) wabi_fixnum_new(&store, 0);

  wabi_val pair = (wabi_val) wabi_cons(&store, zero, nil);

  wabi_val bin = (wabi_val) wabi_binary_leaf_new_from_cstring(&store, "binary");

  wabi_val map = (wabi_val) wabi_map_empty(&store);

  ck_assert_int_gt(wabi_cmp(zero, nil), 0);
  ck_assert_int_gt(wabi_cmp(pair, zero), 0);
