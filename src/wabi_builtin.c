...
wabi_builtin_if(wabi_vm vm)
{
  ...
  ctrl = vm->ctrl;
  self = (wabi_val) ((wabi_cont_call) vm->cont)->combiner;
  env = (wabi_env) ((wabi_cont_call) vm->cont)->env;
...
wabi_builtin_pr(wabi_vm vm)
{
  ...
  ctrl = vm->ctrl;
  while(WABI_IS(wabi_tag_pair, ctrl)) {
    vm->ctrl = wabi_car((wabi_pair) ctrl);
    wabi_pr(vm->ctrl);
    ctrl = wabi_cdr((wabi_pair) ctrl);
...
wabi_builtin_stdenv(wabi_vm vm)
{
  ...
  if(res) return NULL;
  res = wabi_vector_builtins(vm, env);
  if(res) return NULL;
