...
wabi_deque_empty_new(wabi_vm vm)
{
  ...
  if (res) {
    res->size = 0;
    WABI_SET_TAG(res, wabi_tag_deque_empty);
  }
  return res;
}
...
wabi_deque_single_new(wabi_vm vm, wabi_size node_size)
{

  wabi_deque_single res;
  res = (wabi_deque_single) wabi_vm_alloc(vm, WABI_DEQUE_SINGLE_SIZE + node_size);
  if(res) {
    res->size = 0;
    res->nsize = node_size;
...
wabi_deque_single_table(wabi_deque_single d)
{
  return ((wabi_word*) d) + WABI_DEQUE_SINGLE_SIZE;
}


static inline void
wabi_deque_single_set(wabi_deque_single d, wabi_size idx, wabi_val v)
{
